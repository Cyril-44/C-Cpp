...
// ==/UserScript==
// Japanese localization removed — English only
...
// Use English localization only
const currentJson = enJson;
...
class ConfigController {
    register() {
        ...
        // TODO: consolidate this UI registration logic
...
async function modifyStandingsRow(row, results) {
    ...
    // Heuristic: if unrated and rank is not displayed, treat as non-participant (fixes errors in favorite standings)
    if (usernameSpan?.className === "user-unrated" && rankText === "-") {
        userScreenName = null;
    }
    // TODO: prefer to move this logic elsewhere
...
class CalcRatingModel extends EstimatorModel {
    constructor(inputValue, perfHistory) {
        ...
        this.resultDesc = "Expected rating";
    }
    // @ts-ignore
    toggle() {
...
class EstimatorElement {
    ...
    async afterOpen() {
        ...
        /** Update model according to input value */
        ...
        function updateView() {
            ...
            const tweetStr = `AtCoder handle: ${userScreenName}\n${model.inputDesc}: ${roundedInput}\n${model.resultDesc}: ${roundedResult}\n`;
            document.getElementById("estimator-tweet").href = GetEmbedTweetLink(tweetStr, "https://greasyfork.org/scripts/369954-ac-predictor");
        }
    }
    ;
    GetHTML() {
...
function duringVirtualParticipation() {
    ...
    if (!timerText)
        return false;
    const lower = timerText.toLowerCase();
    // consider virtual participation active when timer text does not indicate 'over' or 'ended'
    return !lower.includes("over") && !lower.includes("ended");
}
