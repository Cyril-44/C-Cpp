...
void work() {
    ...
    top = 0;
    for (int i = n; i >= 1; i--) {
        while (top && b[i] > b[st[top]]) { pre[st[top--]] = i + 1; }
        st[++top] = i;
    }
    while (top) { pre[st[top--]] = 1; }
    top = f[0][0] = 0;
    int cnt = 0, mx = 0;
    for (int i = 1; i <= n; i++) {
        if (b[i] > mx) {
        ...
        while (top && b[i] > b[st[top]]) {
            for (int j = 1; j < m; j++) { tr[j][1].update(1, 1, n, 1, st[top], -1); }
            top--;
        }
        st[++top] = i;
        for (int j = 1; j < m; j++) {
...
int main() {
    ...
    int cnt = 0;
    b[++cnt] = n;
    for (int i = id + 1; i <= n; i++) { b[++cnt] = a[i]; }
    for (int i = 1; i < id; i++) { b[++cnt] = a[i]; }
    b[1] = n;
    work();
    for (int i = m; i <= n; i++) { ans = max(ans, f[i][m]); }
    b[cnt = 1] = n;
    for (int i = id - 1; i >= 1; i--) { b[++cnt] = a[i]; }
    for (int i = n; i > id; i--) { b[++cnt] = a[i]; }
    work();
    for (int i = m; i <= n; i++) { ans = max(ans, f[i][m]); }
    cout << ans;
    return 0;
