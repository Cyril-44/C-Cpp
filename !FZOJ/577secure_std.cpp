...
void dfs2(int u, int tp) {
    top[u] = tp, dfn[u] = ++tdfn,
        dfns[tdfn] = bot[u] = u;
    if (hs[u])dfs2(hs[u], tp), bot[u] = bot[hs[u]];
    for (auto each : e[u]) {
        auto v = each.first;
        auto w = each.second;
...
struct mset {
    ...
    int top() {
        while (!q2.empty()) { if (q1.top() != q2.top())break;q1.pop(), q2.pop(); }
...
namespace sgt {
    ...
    bool cov[M];
    void pushupc(int o) { cov[o] = cov[ls] && cov[rs];cov[o] = (cov[o]) || bool(cc[o]); }
    void pushupmx(int o) { mx[o][0] = max(mx[ls][0], mx[rs][0]);mx[o][1] = max(mx[ls][1], mx[rs][1]); }
    ...
    void cover(int o, int lt, int rt, bool op) {
        int l = L[o], r = R[o];
        if (l >= lt && r <= rt) { cc[o] += (op ? 1 : -1);pushupc(o);return; }
    ...
    bool isc(int o, int lt, int rt) {
        if (cov[o])return 1;
        int l = L[o], r = R[o];
        if (l >= lt && r <= rt)return 0;
        if (lt <= mid)if (!isc(ls, lt, rt))return 0;
        if (rt > mid)if (!isc(rs, lt, rt))return 0;
        return 1;
    }
}
#undef ls
...
void ins(int u, int K, bool op) {
    while (u && K >= 0) {
        ...
        sgt::ins(rt, md, K, op);
        int
            l = lower_bound(ss + dfn[tp], ss + dfn[bt] + 1, ss[md] - K) - ss,
            r = upper_bound(ss + dfn[tp], ss + dfn[bt] + 1, ss[md] + K) - ss - 1;
        if (l <= r)sgt::cover(rt, l, r, op);
...
bool getans(int u, int v) {
    ...
    while (top[u] != top[v]) {
        if (d[top[u]] > d[top[v]])
            stkX[++topx] = u, u = fa[top[u]], smxX[topx] = max(0, Qmx(u));
        else
            stkY[++topy] = v, v = fa[top[v]], smxY[topy] = max(0, Qmx(v));
    }
    if (d[u] > d[v])swap(u, v);
    int lca = u, mxcov = 0, lt, len = (s[v] - s[fa[u]]);
    u = fa[top[u]];
    while (u)mxcov = max(mxcov, Qmx(u) - (s[fa[lca]] - s[u])), u = fa[top[u]];
    u = lca;
    if (mxcov < len) {
    ...
    while (topy) {
        u = top[stkY[topy]], v = stkY[topy],
            len = (s[v] - s[fa[u]]), mxcov = smxY[topy];
        if (mxcov < len) {
            lt = upper_bound(ss + dfn[u], ss + dfn[v] + 1, s[fa[u]] + mxcov) - ss;
            if (!sgt::isc(root[u], lt, dfn[v]))return 0;
        }
...
int P[N << 1], K[N << 1];
void work() {
    for (int i = 1;i <= m;i++)in(P[i]), in(K[i]), ins(P[i], K[i], 1);
    int op, x, y;
    while (q--) {
        in(op), in(x);
        if (op != 2)in(y);
        if (op == 1)++m, P[m] = x, K[m] = y, ins(x, y, 1);
        if (op == 2)ins(P[x], K[x], 0);
        if (op == 3)
