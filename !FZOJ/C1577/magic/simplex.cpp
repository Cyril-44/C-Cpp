...
std::vector<std::vector<long double>>
    ...
    tab;
// Compressed tableau (transposed) with
...
void pivot(int x, int y) {
    ...
    for (int j = 0; j <= m + 1; ++j) { tab[x][j] = j == y ? -v : v * tab[x][j]; }
    for (int i = 0; i <= n; ++i) {
        if (i == x) continue;
        v = tab[i][y];
        tab[i][y] = 0;
        for (int j = 0; j <= m + 1; ++j) { tab[i][j] += v * tab[x][j]; }
    }
}
...
bool initialize() {
    ...
    for (int j = 0; j < m; ++j) {
        if (tab[n][j] < -eps) {
            for (int i = 0; i <= n; ++i) { tab[i][m + 1] += tab[i][j]; }
            B[j] = ~B[j];
            ++neg_count;
        }
    }
...
int main() {
    ...
    for (int j = 0; j < m; ++j) {
        for (int i = 0; i <= n; ++i) { std::cin >> tab[i][j]; }
    }
    switch (solve()) {
    case -1:
