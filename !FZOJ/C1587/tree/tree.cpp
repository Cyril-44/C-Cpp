...
int covR[N], covL[N]; // cov: 在 i 下，能作为备选贡献的 mxDep 为 covL[i] ~ covR[i]
...
namespace PntDivide {
    ...
    void dfs1(int u) {
        ...
        vis[u] = true;
        std::vector<int> mxdeps;
        std::vector<std::vector<int>> subtrs;
        sons[u].reserve(g[u].size()), mxdeps.reserve(g[u].size()), subtrs.reserve(g[u].size());
        int mxid = -1, mx2id = -1;
        for (int v : g[u]) if (!vis[v]) {
            mxdep = -1, dfs3(v), subtrs.emplace_back().swap(nodes);
            if (mxid == -1 || mxdep >= mxdeps[mxid]) mx2id = mxid, mxid = sons[u].size();
            else if (mx2id == -1 || mxdep >= mxdeps[mx2id]) mx2id = sons[u].size();
            sons[u].push_back(v);
            mxdeps.push_back(mxdep);
        }
        if (mxid == -1) return;
        if (!a[u]) umax(mxDis[u], mxdeps[mxid]);
        if (mx2id == -1) return;
        for (int i = 0; i < sons[u].size(); i++) {
            int dis = i == mxid ? mxdeps[mxid] : mxdeps[mx2id];
            for (int vv : subtrs[i]) if (!a[vv])
                umax(mxDis[vv], dep[vv] + dis);
        }
        for (int v : sons[u]) dfs1(v);
    }
    void dfs2(int u) {
        vis[u] = true;
        std::vector<int> mxids, mx2ids;
        int mxv = -1, mx2v = -1;
        std::vector<std::vector<int>> subtrs;
        for (int v : g[u]) if (!vis[v]) {
            mxdep = -1, dfs3(v), subtrs.emplace_back().swap(nodes);
            if (mxdep > mxv) mx2v = mxv, mx2ids = mxids, mxv = mxdep, mxids = {v};
            else if (mxdep == mxv) mxids.push_back(v);
            else if (mxdep > mx2v) mx2v = mxv, mx2ids = {v};
            else if (mxdep == mx2v) mx2ids.push_back(v);
        }
        for (int v : sons[u]) dfs2(v);
    }
...
static struct DequeNonEQ : public DequeBase {
    void push(int i) {
        ...
        For(i, 1, t) { // 处理跨树（经过环边）
            ...
            flgR[deq.front()] = true;
            covR[i] = dneq.front();
            umax(mxExDis[i], arrR[deq.front()] - i);
        }
        ...
        roF(i, t, 1) {
            ...
            flgL[deq.front()] = true;
            covL[i] = dneq.front();
            umax(mxExDis[i], arrL[deq.front()] - (t - i));
        }
