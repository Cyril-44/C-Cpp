...
显然这个时间复杂度时可以优化到 $cal(O)(n^2)$ 的。无法通过本题。#pagebreak()

考虑套路性的使用长链剖分进行优化。#pause

首先我们对式子进行一些改写。

$ "s"f_(u,i) &= sum_(v) "s"f_(v,i-1) (|"ch"_u| - 1)! product_(v' != v) "s"f_(v', K-2) \ #pause 
&= (|"ch"_u|)! product_(v) "s"f_(v, K-2) sum_v 1/f_(v,i)
$

这样的话式子就优美多了。#pause
我们发现，$"s"f_(u,i)$ 贡献事实上分为与 $i$ 有关和无关的两个部分。#pause
于是我们的长剖继承，就可以将 $f$ 继承下来然后进行一个整体乘一个系数的操作。
]
