...
现在，我们就考虑推广这个做法。我们观察到，chkmin 操作真的很强，它会抹掉 $a_i$ 原本的特征。思考一个 $a_i$ 被实际上 chkmin 了（满足 $a_i > v$）之后，会怎么样？\ #pause
...
后面至少有 $n-x$ 个 $1 ==> $ 前面至多 $x$ 个 $1$. 但是前面有 $x$ 个 $-1$, 又根据 $forall "s"b_i >= 0$ 我们能够知道前面至少有 $x$ 个 $1$, 所以事实上就是前面有 $x$ 个 $-1$ 和 $1$, 后面有 $n-x$ 个 $-1$ 和 $1$. 我们就得到了优美的 $"LIS"$ 形式。
...
, 那么任何一种循环移位都无解。\
构造证明：只需要取 $b_i=0$ 的位置、该位置前面所有的 $-1$ 和后面所有的 $1$ 的位置（这些位置天然递增），构成的 $"IS"$ 长度就是 $>n$ 的。\ #pagebreak()
