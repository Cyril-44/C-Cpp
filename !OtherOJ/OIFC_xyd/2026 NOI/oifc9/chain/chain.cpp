...
int baseid[N], dep[M], pre[M], fa[M][LG+1];
...
void dfs(int u) { // 从下往上搞出来转移矩阵
    ...
    roF(i, tr[u].sz()-2, 0) sufsum[tr[u][i]] += sufsum[tr[u][i+1]];
    For(i, 0, tr[u].sz()-1) f[tr[u][i]][0] = {{{i ? presum[tr[u][i-1]] : 0, sufsum[tr[u][i]]}, {presum[tr[u][i]], i!=tr[u].sz()-1 ? sufsum[tr[u][i+1]] : 0}}};
}
...
int main() {
    ...
    For(_q, 1, q) {
        ...
        if (x == y) fout << tr[x].dis << '\n';
            ...
            std::cout << "\ttrans vgx:" << vgx << ", vgy:" << vgy << ", vfx:" << vfx << ", vfy:" << vfy << '\n';
            int lca = fa[x][0];
            assert(pre[y]);
