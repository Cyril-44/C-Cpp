...
ll brute(ll A, ll B) {
    ...
    if (A < 0) {
        for (ll x = 0; x <= B; ++x) { st.insert(cum(x) ^ 0LL); }
    }
    return st.size();
}
...
ll solve(ll l, ll r) {
    ...
    ll i_min2 = A + ((2 - mA + 4) % 4);
    ll num3 = 0;
    ll k_low3 = 0, k_high3 = 0;
    ll v_min3 = 0, v_max3 = 0;
    if (i_min2 <= B) {
        int mB_i = mB;
        ll i_max2 = B - ((mB_i - 2 + 4) % 4);
...
int main() {
    ...
    for (int i = 1; i < (1 << B); i++) pre[i] = pre[i - 1] ^ i;
    int T;
    ...
    while (T--) {
        ...
        if (l == r)
            ...
            int now = 64 - __builtin_clzll(r);
            printf("%llu\n", (1ull << now) - ((1ull << now) - r - 1 >> 1));
        } else if (r <= 1000) {
            memset(vis, 0, sizeof vis);
            for (unsigned i = l; i <= r; ++i)
                for (unsigned j = i; j <= r; ++j) vis[pre[j] ^ pre[i - 1]] = true;
            unsigned cnt = 0;
            for (unsigned i = 0; i < (1 << B); i++)
                if (vis[i]) ++cnt;
            printf("%u\n", cnt);
        } else printf("%lld\n", Try::solve(l, r));
    }
