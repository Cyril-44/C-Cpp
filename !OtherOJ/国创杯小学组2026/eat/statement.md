...
某一天她的*开心值*为 $p$，*幸运数*为 $x$，当天剩余的 $k$ 格巧克力*美味度*从左向右假设为 $s_0, s_1, \cdots, s_{k-1}$，小可可计算出第 $i$ 格巧克力的*契合度* $b_i$ 等于 $s_i \times s_{(i+p) \bmod k}$。
...
接下来 $m$ 行，每行两个正整数 $p,x$，表示第 $1\sim m$ 天小可可给你的 $p,x$。
...
对于所有测试数据，均有：$p,n,m \le 10^6$，$x \le 10^{18}$，$a_i \le 10^9$。
