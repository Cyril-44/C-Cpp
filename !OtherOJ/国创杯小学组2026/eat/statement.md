...
小可可计划每天吃掉巧克力的左端或者右端的连续一部分。具体来说，她每天都有一个开心值 $p$，然后她会将剩余的巧克力的美味度从左至右顺时针首尾相接写成一个环，并计算第 $i$ 格巧克力与从这一格巧克力开始顺时针走 $p$ 格所到达的巧克力的美味度的乘积，作为第 $i$ 格巧克力的契合度 $b_i$。 *这也意味着每天每格巧克力的契合度都有可能与前一天不一样。* 然后她会在所有契合度中选择一个作为当天的幸运数 $x$。
...
小可可记录了前 $m$ 天中，她每天的开心值 $p$ 和幸运数 $x$，但是她已经忘了巧克力是怎样吃掉的了。请你帮助她回忆起这 $m$ 天里她是如何吃巧克力的吧！
...
|:-|:-:|-:|
|图例左侧|图例中部|图例右侧|
|吃法记录。|$a,b$ 如题意，**粉数字为 $x$**。|小可可会将美味度排成环，|
|黑字表示第几天。|带箭头的线段表示一种吃法,|每次走 $p$ 格，从*红*到达*浅蓝*，|
|红字为当天吃法。|**标红的线段为当日选择的吃法。**|*浅蓝*乘*红*代表*红*的契合度。|
...
第三天 $p=4$，在大小为 $3$ 的环上顺时针走 $4$ 格实际上相当于顺时针走了 $1$ 格，于是计算出巧克力契合度为 $\{12,6,8\}$，可以发现此时有两种吃法：从左端开始向右吃到格 $2$ 以及从右端开始向左吃到格 $2$，都是吃得最少的，根据规则小可可选择从左端开始吃巧克力，也就是从左端开始向右吃 $2$ 格。
...
特殊性质 B：每天最左端或者最右端 $10$ 格巧克力中，总是存在一个幸运格。
