...
use vars qw($VERSION @FINC %FINC $STRICT $CACHE $NAMED_SUBS %OPS $DEBUG);
$VERSION = '2.16';
...
my $last_package_name = "TFpkg0000001";
my %file_to_package;
...
sub namespace {
  ...
  my $ret = $file_to_package{$fpath};
  return $ret if $ret;
  ++$last_package_name;
  $ret = $last_package_name;
  $file_to_package{$ret} = $fpath;
  return $ret;
  #return "TF$fpath";
}
...
sub compile {
  ...
  open XF1111, "<${fpath}" or croak "unable to read '$fpath': $!";
  my $source = do { local $/; <XF1111> };close(XF1111);
...
sub send_header {
  my $self = shift;
  return if $self->{header_sent}++ || $self->{forced_no_header};
