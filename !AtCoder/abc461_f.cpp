...
// Dynamic_ModInt::value_type Dynamic_ModInt::value = 998244353;
...
Mint a;
vector<int64_t> getfacs(int64_t n) {
    vector<int64_t> facs;
    int lmt = sqrt(n);
    for (int i = 1; i <= lmt; i++)
        if (n % i == 0) facs.push_back(i);
    int i = facs.size() - 1;
    if (1ll * facs[i] * facs[i] == n) --i;
    for (; i >= 0; i--)
        facs.push_back(n / facs[i]);
    return facs;
}
inline void solveSingleTestCase() {
    ...
    cin >> n;
    auto facs = getfacs(n);
    map<int64_t, vector<pair<Mint,Mint>>> f;
    auto has = [&](int64_t v) {
        auto it = lower_bound(facs.begin(), facs.end(), v);
        return it != facs.end() && *it == v;
    };
    f[1] = {{0,1},{1,1}};
    facs.erase(facs.begin());
    for (int64_t fac : facs) {
        auto g = f;
        for (auto &[v, dp] : f) {
            int64_t nv = v * fac;
            if (!has(nv)) continue;
            if (g[nv].size() < dp.size() + 1) g[nv].resize(dp.size() + 1);
            for (int i = 0; i < (int)dp.size(); i++) {
                auto &cur = dp[i], &upd = g[nv][i+1];
                upd.second += cur.second;
                upd.first += (cur.first + cur.second * fac);
            }
        }
        g.swap(f);
        // for (auto &[v, dp] : f) {
        //     for (int i = 0; i < (int)dp.size(); i++) {
        //         auto &cur = dp[i];
        //         // cout << v << ' ' << i << ' ' << cur.first << ' ' << cur.second << endl;
        //     }
        // }
        // cout << "==================" << endl;
    }
