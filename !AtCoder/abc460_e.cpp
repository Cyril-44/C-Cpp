...
inline void solveSingleTestCase() {
    ...
    int k = 1; i128 base = 1;
    Mint ans = 0;
    for (; base <= n; base*=10, k++) {
        i128 g = std::__gcd(base*10-1, (i128)m);
        i128 a=(base-1)/g, b=m/g;
        // cerr << "Digit = " << k << ", " << std::min(base*9-(k==1), n-base+1) << endl;
        ans += Mint(int64_t(std::min(base*9, n-base+1) % MOD)) * (n / b);
    }
    cout << ans << '\n';
}
int main() {
