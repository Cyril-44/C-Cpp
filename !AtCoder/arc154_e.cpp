...
inline void solveSingleTestCase() {
    ...
    cin >> n >> m;
    For(i, 1, n) cin >> p[i];
    Mint base = Mint(n * (n+1ll) / 2);
    Mint dino = base ^ m, invdino = 1 / dino;
    Mint mid = Mint(n+1) / 2;
    Mint ans = 0;
    for (int i = 1; i <= n; i++) {
        Mint now = base - Mint(i) * Mint(n - i + 1);
        Mint pprob = (now ^ m) * invdino, lprob = 1 - pprob;
        ans += Mint(i) * i - p[i] * (i * pprob + mid * lprob);
    }
    cout << ans * dino;
}
int main() {
