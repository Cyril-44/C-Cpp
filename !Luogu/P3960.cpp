...
struct Node {
    ...
    inline void zig() {
        auto u = rs;
        auto anc = fa;
        if (anc) (anc->ls == this ? anc->ls : anc->rs) = u;
        u->fa = fa;
        fa = u;
        rs = u->ls;
        if (u->ls) u->ls->fa = this;
        u->ls = this;
        pushup();
        u->pushup();
    }
    inline void zag() {
        auto v = ls;
        auto anc = fa;
        if (anc) (anc->ls == this ? anc->ls : anc->rs) = v;
        v->fa = fa;
        fa = v;
        ls = v->rs;
        if (v->rs) v->rs->fa = this;
        v->rs = this;
        pushup();
        v->pushup();
    }
    inline void rotate() { (fa->ls == this) ? fa->zag() : fa->zig(); }
    ...
    */
    ...
    void splay(Node *u, Node *tgt) { // 伸展 u 使 u->fa = tgt（默认旋到顶）
        while (u->fa != tgt) {
            ...
            if (u->fa->fa != tgt) (u->fa->ls == u) == (u->fa->fa->ls == u->fa) ? u->fa->rotate() : u->rotate();
            u->rotate();
        }
        if (!tgt) root = u; // 旋到根了
    }
    ...
    inline void erase() {
        if (root->ls) {
            ...
            root->ls->rs = root->rs;
            if (root->rs) root->rs->fa = root->ls;
            root = root->ls;
        } else if (root->rs)
            root = root->rs;
        else
            return root = nullptr, void();
        root->fa = nullptr;
        if (root->ls) root->ls->pushup();
        if (root->rs) root->rs->pushup();
        root->pushup();
    }
    inline LL erase(int p) {
        ...
        splay(u);
        if (u->cnt == 1) {
            logprint("+ Erased (Case 0): sz=%d, v=%lld\n", u->size, u->val);
            return erase(), u->val;
        }
        if (u->lsz() + 1 == p) { // p 在最左边
            Node *restr = Splay_Data::alloc(nullptr, u->rs, u, u->cnt - 1, u->val + 1);
            if (u->rs) u->rs->fa = restr;
            u->cnt = 1, u->rs = restr;
            logprint("+ Erased (Case 1 left): sz=%d, v=%lld [Rs: sz=%d, v=%lld]\n", u->size, u->val, u->rs->size, u->rs->val);
            return erase(), u->val;
        }
        if (u->lsz() + u->cnt == p) { // p 在最右边
            Node *restl = Splay_Data::alloc(u->ls, nullptr, u, u->cnt - 1, u->val);
            restl->pushup();
            if (u->ls) u->ls->fa = restl;
            u->ls = restl, u->val += u->cnt - 1, u->cnt = 1;
            logprint("+ Erased (Case 1 right): sz=%d, v=%lld [Ls: sz=%d, v=%lld]\n", u->size, u->val, u->ls->size, u->ls->val);
            return erase(), u->val;
        }
        Node *restl = Splay_Data::alloc(u->ls, nullptr, u, p - u->lsz() - 1, u->val),
             *restr = Splay_Data::alloc(nullptr, u->rs, u, u->lsz() + u->cnt - p, u->val + p - u->lsz());
        u->cnt = 1, u->val += p - u->lsz() - 1;
        if (u->ls) u->ls->fa = restl;
        if (u->rs) u->rs->fa = restr;
        u->ls = restl, u->rs = restr;
        logprint("+ Erased (Case 1 mid): sz=%d, v=%lld [Ls: sz=%d, v=%lld] [Rs: sz=%d, v=%lld]\n", u->size, u->val, u->ls->size, u->ls->val, u->rs->size, u->rs->val);
        return erase(), u->val;
    }
    inline void push_back(LL x) {
        if (!root) { // Specific: when empty
            root = Splay_Data::alloc(nullptr, nullptr, nullptr, 1, x);
            return;
        }
        Node *curr = last(root);
    ...
    inline void dbg(Node *u, int dep) {
        ...
        logprint("[%lld,%lld] sz=%d l=%d\n", u->val, u->val + u->cnt - 1, u->size, u->cnt);
        dbg(u->rs, dep + 2);
    }
} last_col, row[N];
int main() {
#ifdef DEBUG
    freopen("P3960_15.in", "r", stdin);
    freopen("P3960.out", "w", stdout);
    freopen("P3960.txt", "w", stderr);
#endif
    int n, m, q;
    scanf("%d%d%d", &n, &m, &q);
    for (int i = 1; i <= n; i++) {
        row[i].init(m - 1, (i - 1ll) * m + 1);
        last_col.push_back(i * 1ll * m);
    }
    // puts("Last column: ");
    ...
    for (int x, y, qi=1; qi<=q; ++qi) {
        ...
        if (y != m) {
            LL mr = row[x].erase(y);
            // logprint("Row %d erased %d (id=%lld)\n", x, y, (row[x].dbg(4), mr));
            row[x].push_back(lc);
            // logprint("Row %d pushed (id=%lld)\n", x, (row[x].dbg(4), lc));
