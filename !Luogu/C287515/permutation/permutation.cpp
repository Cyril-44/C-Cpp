...
Mint C[N][N];
void* __init_C = [](const int n = 5000){
    For (i, 1, n) {
        C[i][0] = C[i][i] = 1;
...
bool g[N][N]; // g_i_j = true -> p[i] < p[j];
int indeg[N];
#define less_than(i, j) do { if (g[j][i]) return (cout << "0\n"), void(); g[i][j] = true; } while (0)
inline void solveSingleTestCase() {
    ...
    For (i, 1, n) {
        ...
        switch (op) {
        case 0:
            For (j, 1, i-1) less_than(i, j);
            break;
        case 1:
            For (j, 1, i-1) less_than(j, i);
            break;
        case 2:
            For (j, i+1, n) less_than(i, j);
            break;
        case 3:
            For (j, i+1, n) less_than(j, i);
            break;
        default: abort();
        }
    }
