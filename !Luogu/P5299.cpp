#include <bits/stdc++.h>
constexpr int N = 1, MOD = 998244353;
...
int main() {
    ...
    while (T--) {
        ...
        { // 计算：A 里面选 k-1 个最大的，B 选一个
            ...
            for (int i = 1; i <= n; i++)
                for (int j = 0, upper = n - std::max(i, k-1); j <= upper; j++) // 再额外选 j 个
                    ffa[k-1 + j] += fa[i][k-1] * C[n-i][j];
            Mint fb[N], gb[N]; // B 选 j 个的答案和，方案数
            gb[0] = 1;
            for (int i = 1; i <= n; i++)
                for (int j = i; j >= 1; j--)
                    fb[j] += fb[j-1] + gb[j-1] * b[i], gb[j] += gb[j-1];
            // for (int i = 0; i <= n; i++)
            //     for (int j = n - i; j >= 0; j--)
            //         ans[i+j] += fa[i] * fb[j];
        }
        { // 计算：A 里面选 k-x 最大的，B 选 x 个
