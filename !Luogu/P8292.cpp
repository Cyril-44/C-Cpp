...
#include <type_traits>
#include <set>
constexpr int N = 2001, DVD = 43, M = 14, P = 303;
...
int h[N];
bool np[N];
int primes[P+1], pmap[N];
int cnt[P][1<<M];
Mint pw2[N], ipw2[N], g[P][1<<M], f[1<<M];
...
int main() {
    ...
    for (int i = 1; i < N; i++) pw2[i] = pw2[i-1] + pw2[i-1], ipw2[i] = ipw2[i-1] * i2;
    int n, m;
    scanf("%d", &n);
    ...
    for (int v = 1; v < N; v++) {
        ...
        if (x > 1 && x < DVD) facs.push_back(x), x = 0;
        else x = pmap[x];
        int mask = 0;
        for (int p : facs) mask |= 1 << pmap[p];
        fprintf(stderr, "v=%d: x=%d, mask=%X, +=%d\n", v, x, mask, h[v]);
        for (int s = 0; s < (1<<M); s++)
            if (!(s & mask)) cnt[x][s] += h[v];
    }
    for (int i = 0; i < P; i++) {
        fprintf(stderr, "Prime %3d: ", primes[i+1]);
        for (int s = 0; s < (1<<M); s++)
            if (cnt[i][s])
                fprintf(stderr, "(%d,%d)", s, cnt[i][s]);
        fprintf(stderr, "\n");
    }
    for (int s = 0; s < (1<<M); s++)
        f[s] = pw2[cnt[0][s]];
    ...
    for (int c; m--; ) {
        ...
        for (int s = 0; s < (1<<M); s++)
            if ((s & mask) == mask)
                if (__builtin_popcount(s ^ mask) & 1) ans -= nf[s];
                else ans += nf[s];
        printf("%d\n", ans);
    }
}