...
class SegTr {
    ...
    void merge(int& u, int v, int l, int r) {
        if (!u || !v) { u |= v; return; }
        if (l == r) { tr[u].val *= tr[v].val; return; }
...
public:
    ...
    void merge(int u, int v) { merge(rt[u], rt[v], 1, n); }
    void update(int u, Mint m) { if (rt[u]) tr[rt[u]].pull(m); }
    Mint operator()(int u) { return tr[u].val; }
...
void dfs(int u, int fa) {
    ...
    for (int v : g[u]) if (v != fa) {
        ...
        if (a[u] == a[v]) {
            ...
            f.apply(u, a[v], (f(u, a[u]) + f(u, a[v])) * (f(v, a[u]) + f(v, a[v])) + f(u, a[u]));
        }
    }
}
int main() {
