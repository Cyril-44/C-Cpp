...
pair<LINE, long long>
max(LINE a, LINE b) // 对两个一次函数取x=0时的最值，同时给出max的选取不发生变化的最大值C(即 x>C 时max变成另一个)
{
...
void dfs(int rot, int lt, int rt, long long w) {
    if (w > a[rot].x) // 如果增量使得子树内有max发生变化则递归下去重构
    {
        long long t = tag[rot] + w;
        tag[rot] = 0;
        int mid = (lt + rt) >> 1;
        dfs(rot << 1, lt, mid, t), dfs(rot << 1 | 1, mid + 1, rt, t);
        a[rot] = a[rot << 1] + a[rot << 1 | 1];
    } else
        upd(rot, w);
}
