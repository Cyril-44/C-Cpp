...
pair<int, int> calc() {
    ...
    for (int i = 1; i < (int)ord.size(); i++) {
        while (s[tp - 1] != ord[0] && 1ll * (y[ord[i]] - y[s[tp - 1]]) * (x[ord[i]] - x[s[tp - 2]]) >=
                                          1ll * (y[ord[i]] - y[s[tp - 2]]) * (x[ord[i]] - x[s[tp - 1]]))
            tp--;
        s[tp++] = ord[i];
    }
    for (int i = (int)ord.size() - 2; i >= 0; i--) {
        while (s[tp - 1] != ord.back() && 1ll * (y[s[tp - 1]] - y[ord[i]]) * (x[s[tp - 2]] - x[ord[i]]) >=
                                              1ll * (y[s[tp - 2]] - y[ord[i]]) * (x[s[tp - 1]] - x[ord[i]]))
            tp--;
        s[tp++] = ord[i];
    }
    tp--;
    ...
    int a = s[cur], b;
    if (dist(s[cur], s[0]) > dist(s[cur], s[tp - 1]))
        b = s[0];
    else
        b = s[tp - 1];
