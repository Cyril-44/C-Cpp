...
inline void solveSingle() {
    ...
    For(i, 1, n) aval[i].reset(), aval[i].set(i);
    roF(i, n, 1) for (int j : g[i]) aval[j] |= aval[i];
...
#define bswap(a, x, y) do {                                     \
    int bl = (a[x]-1) / B, br = (a[y]-1) / B;                   \
    if (bl > br) std::swap(bl, br);                             \
    For(i, bl+1, br) a##ge[i].flip(x), a##ge[i].flip(y);        \
    std::swap(p##a[a[x]], p##a[a[y]]);                          \
    std::swap(a[x], a[y]);                                      \
} while (0)
    for (int o, x, y, z; q--; ) {
        scanf("%d%d%d", &o, &x, &y);
        ...
        case 3: { // query max_{x->i and y<=a_i<=z} b_is
            ...
            For(p, blkL * B + 1, y) sat.reset(pb[p]);
            For(p, blkR * B + 1, z) sat.set(pb[p]);
            sat &= aval[x];
            int blk = -1;
            For(j, 0, lastBitsetBlock)
                while (sat(j) & bge[blk+1](j)) { if (++blk == lastblock) break; }
            if (blk == -1) puts("0");
            else roF(i, B * (blk+1), B * blk + 1)
                if (sat[pb[i]]) { printf("%d\n", i); break; }
        } break;
        }
    }
}
